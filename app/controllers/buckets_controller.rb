class BucketsController < ApplicationController
  ...
  def edit
    ...
    authorize @bucket
    @translation_keys = @bucket.translations.map { |translation| translation.translation_key }.uniq.sort
    # if bucket.kind == "d"
  ...
  def update
      ...
      @bucket.project.languages.each do |language|
        if @bucket.kind == "s"
          ...
          unless translation.nil?
            ...
            translation.text = params[:translation_keys][key][language.language_key].strip unless translation.translation_key == params[:translation_keys][key][language.language_key]
          end
        elsif @bucket.kind == "d"
          ...
          translation.text = params[:translation_keys]['sub_bucket_' + @sub_bucket.id.to_s][key][language.language_key].strip unless translation.translation_key == params[:translation_keys]['sub_bucket_' + @sub_bucket.id.to_s][key][language.language_key]
    ...
    if @bucket.kind == "s"
      unless params[:translation_keys][:new_translation_key][:value] == ""
        @bucket.project.languages.each do |language|
          Translation.create(language: language, bucket: @bucket, translation_key: params[:translation_keys][:new_translation_key][:value], text: params[:translation_keys][:new_translation_key][language.language_key] != "" ? params[:translation_keys][:new_translation_key][language.language_key].strip : nil)
        end
      end
      if params[:languages]
        @languages = Language.where(language_key: params['languages'].keys)
